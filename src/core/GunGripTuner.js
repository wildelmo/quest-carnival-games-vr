...
/**
 * GunGripTuner — tune the six-shooter's hold pose FROM INSIDE the headset,
 * the same workflow that dialled in the dart grip (see DartGripTuner):
 *
 *   1. Hold a gallery gun (any grabbable with holdPose 'pistol').
 *   2. SQUEEZE THE GRIP on your empty other hand — a numbers panel fades
 *      in above the gun and that hand's thumbstick nudges the gun around
 *      inside your grip:
 *        stick alone      →  x: side-to-side (up)   y: forward/back (fingers)
 *        stick + TRIGGER  →  y: out of palm (palm)  x: barrel pitch (noseUp°)
 *        A / X button     →  reset to the shipped numbers
 *   3. Values apply to every gun live and persist in localStorage — read
 *      them off the panel and bake keepers into ShootingGalleryGame's
 *      GUN_HOLD once the grip feels right, then flip TUNER_ENABLED off.
 *
 * Only the XR half of holdQuat is touched: the desktop hold (barrel down
 * the view) is its own thing and never needs this. While the tuner owns a
 * thumbstick it sets hand.stickClaimed, which Locomotion honours.
 */
...
const DEAD = 0.15;
const LIMITS = {
  palm: [-0.04, 0.12], fingers: [-0.06, 0.12], up: [-0.08, 0.10], noseUp: [-10, 90],
};

const _v1 = new THREE.Vector3();

export class GunGripTuner {
  /**
  ...
  /** all pistol-held grabbables (the gallery guns) */
  #targets() {
    return this.grabbables.items.filter((g) => g.holdPose === 'pistol');
  }
  ...
  #apply() {
    ...
    for (const g of this.#targets()) {
      ...
      // only the XR swing changes — keep the gun's own desktop hold intact
      const q = noseOutHoldQuat(v.noseUp);
      g.holdQuat = { ...g.holdQuat, xr: q.xr, noseUpDeg: v.noseUp };
    }
    this._dirty = true;
  }
  ...
  #update(dt, t) {
    ...
    // who's holding a gun, and which hand is free to tune with?
    ...
    for (const hand of this.input.hands) {
      ...
      if (g?.holdPose === 'pistol') heldHand = hand;
    ...
    // panel stays up while the gun is held once the tuner has been used
  ...
  #draw(active) {
    ...
    c.fillText('GUN GRIP TUNER', 24, 44);
    ...
    const rows = [
      ...
      ['up', v.up.toFixed(3)],
      ['barrel', `${v.noseUp >= 0 ? '+' : ''}${v.noseUp.toFixed(1)}°`],
    ];
    ...
    c.fillStyle = '#9adf9a';
    c.fillText('stick: side/fwd   +trigger: palm/barrel', 24, 266);
